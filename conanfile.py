...
set(CMAKE_MODULE_PATH ${CMAKE_BINARY_DIR} ${CMAKE_MODULE_PATH})''')
        ...
        if self.options.shared:
        	tools.replace_in_file("ZLToolKit/CMakeLists.txt", ''' add_library(${CMAKE_PROJECT_NAME}_shared SHARED ${SRC_LIST})
    ...
    install(TARGETS ${CMAKE_PROJECT_NAME}_shared  ARCHIVE DESTINATION ${INSTALL_PATH_LIB} LIBRARY DESTINATION ${INSTALL_PATH_LIB}) ''', '''  ''')
        	...
        	tools.replace_in_file("ZLToolKit/CMakeLists.txt", ''' add_library(${CMAKE_PROJECT_NAME}_static STATIC ${SRC_LIST})
